...
> **One SKILL file, any AI Agent can call Temar payment capabilities via X402 protocol.**

## What This Is

This file is an AI Agent-readable skill instruction. Any AI Agent that supports the X402 protocol can, after reading this file, call Temar's payment, payout, and balance query capabilities via HTTP + stablecoin pay-per-use.
...
└──────────────┬────────────────┘
               ...
               ▼
┌───────────────────────────────┐
│   Temar Public API            │  ← Already deployed
│   /api/v2/digital/*           │
└───────────────────────────────┘
```
...
|--------|----------|------------|-------------------|
| `POST` | `/v1/payment/create` | Create payment order (collect) | **Dynamic** — X402 amount = actual transfer amount, payTo = Temar receiveAddress |
...
> **X402 only for transfer operations:** `payment.create` and `payout.create` use X402 as the actual business transfer. The Agent pays USDC directly to the address returned by Temar API (receiveAddress) using the user-authorized wallet.
...
> ⚠️ **Do NOT include a recipient address** — The receiveAddress is generated by Temar system.

The server internally calls Temar API to create the order, obtains `receiveAddress`, and returns `402`:
...
> `payTo` = The receive address generated by Temar for this order (from the API response's `receiveAddress` field)
...
Server verifies on-chain proof → calls Temar API → returns 200:
...
This Skill's position in the Temar payment protocol stack:
...
| L0 | **MCP (TemarAIMCP)** | **Capability Execution** | ✅ Live |
