...
- Shared domain guides such as [Auth Signing](../domains/shared/auth-signing.en.md), [Error Handling](../domains/shared/error-handling.en.md), and [Response Conventions](../domains/shared/response-conventions.en.md) describe rules reused across capabilities.
- Domain guides such as [Crypto Overview](../domains/crypto/overview.en.md) explain published runtime capability flows for a specific domain.
...
- MCP is the capability base beneath the current published public docs.
- `skill`, `x402`, and future protocol surfaces should route into MCP rather than maintain separate business cores.
- The current `docs/skill` tree should be read as the public guidance layer for MCP-backed capability usage.

## Available domains

- Shared: Cross-capability rules that apply before or alongside any business flow, including signing, response conventions, and error handling.
- Crypto: The current published domain. This domain includes payment, payout, order query, webhook, and balance capability guidance for the published digital asset endpoints.
