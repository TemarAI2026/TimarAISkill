...
- Hub documents such as [Integration Router](./integration-router.md) help you choose a reading path.
- Shared domain guides such as [Auth Signing](../domains/shared/auth-signing.md), [Error Handling](../domains/shared/error-handling.md), and [Response Conventions](../domains/shared/response-conventions.md) describe rules reused across domains.
- Domain guides such as [Crypto Overview](../domains/crypto/overview.md) explain business flows for a specific domain.
- Reference documents such as [crypto endpoints](../references/crypto/endpoints.md), [request and response models](../references/crypto/request-response-models.md), and [shared headers](../references/shared/headers.md) provide implementation details.
...
- Fiat: A reserved future domain. Its entry point already exists at [Fiat README](../domains/fiat/README.md), but no public fiat flow, field set, or status mapping is published yet.
